...
Thumbnail Generator is a static page, so hosting it yourself is one container serving
...
push and published as `ghcr.io/stoatworks-labs/thumbnail-generator`:

```bash
docker run -d --name thumbnail-generator --restart unless-stopped -p 8528:80 ghcr.io/stoatworks-labs/thumbnail-generator:latest
```
...
**Unraid.** Search Community Applications for *Thumbnail Generator* — the template is
[`templates/thumbnail-generator.xml`](https://github.com/stoatworks-labs/stoatworks-unraid/blob/main/templates/thumbnail-generator.xml)
